...
class College_model extends CI_Model
{
     ...
     */
    public function countFilteredClg($search, $loc, $ownerShip, $rankCategory,$courseid,$value,$categoryid='')
    {
        $this->db->select('*');
   ...
   public function getFilteredClg($clgname, $start, $limit, $order, $dir, $loc, $ownerShip, $rankCategory, $courseid, $value, $categoryid)
{
    $this->db->select('c.id, c.package_type, c.application_link, c.is_accept_entrance, c.logo, c.title, c.banner, c.estd, ci.city, COALESCE(g.image, "") AS gallery_image, (CASE WHEN c.package_type = "featured_listing" THEN 0 ELSE 1 END) AS sort_order');
    $this->db->from('college c');
    ...
    {
        $this->db->select('c.id, c.package_type,c.application_link, logo,c.is_accept_entrance, c.title, banner, estd, ci.city, COALESCE(g.image, "") AS gallery_image, (CASE WHEN c.package_type = "featured_listing" THEN 0 ELSE 1 END) AS sort_order');
        $this->db->from('college c');
        $this->db->join('city ci', 'ci.id = c.cityid', 'left');
        $this->db->join('gallery g', 'g.postid = c.id', 'left');
        $this->db->where('c.is_deleted', '0');
    ...
    {
        $this->db->select('c.id, c.title, c.slug, c.description,c.accreditation, c.address, c.web, c.estd, COALESCE(g.image, "") AS image,,ct.city,st.statename,c.notification,c.notification_link');
        $this->db->from('college c');
        $this->db->join('gallery g', 'g.postid = c.id', 'left');
...
{
    $this->db->select('c.id,c.logo,c.banner, c.cityid,c.is_accept_entrance,c.college_typeid, c.title,c.application_link, c.what_new, c.categoryid, c.description, c.accreditation, c.package_type, c.logo, GROUP_CONCAT(ca.catname) AS catname, c.banner, c.estd, ci.city, co.country, COALESCE(g.image, "") AS image, ct.name');
    $this->db->from('college c');
    $this->db->join('city ci', 'ci.id = c.cityid', 'left');
